class Form{
    constructor(){
       this.instruction=createElement("h2")
       this.input = createInput("");
       this.button = createButton('Play');
       this.greeting = createElement('h2');
       this.title = createElement('h2');
       this.reset = createButton('Reset');
    }
    hide() {
        this.greeting.hide();
        this.button.hide();
        this.input.hide();
        this.title.hide();
        this.instruction.hide();
    }
    display() {
        this.title.html("FRUIT CATCHER");
        this.title.position(windowWidth/3.5, 50);
        this.title.style('font-size', '70px');
        this.title.style('color', 'cyan');
        this.instruction.html("enter your name")
        this.instruction.position(windowWidth/2.25,350)
        this.instruction.style('color','cyan')
        this.input.position(windowWidth/2.3,400);
        this.input.style('width', '200px');
        this.input.style('height', '20px');
        this.input.style('background', 'cyan');
        this.button.position(windowWidth/2.3,500);
        this.button.style('width', '200px');
        this.button.style('height', '40px');
        this.button.style('background', 'cyan');
        this.reset.position(windowWidth/7, 660);
        this.reset.style('width', '100px');
        this.reset.style('height', '30px');
        this.reset.style('background', 'cyan');

        this.button.mousePressed(() => {
            this.input.hide();
            this.button.hide();
            this.instruction.hide();
            player.name = this.input.value();
            playerCount += 1;
            player.index = playerCount;
            player.update();
            player.updateCount(playerCount);
            this.greeting.html("Hello " + player.name)
            this.greeting.position(windowWidth/3,250);
            this.greeting.style('color' , 'white');
            this.greeting.style('font-size', '100px');
        });

        this.reset.mousePressed(() => {
            //add code to reset the values of the gameState and the playerCount nodes to 0 in the database
            game.update(0)
            player.updateCount(0)
            var playerCountRef=database.ref('players');
            playerCountRef.remove();

        });

    }
}